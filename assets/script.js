class ServerNotification {
    ...
    start() {
        if (this.sse) {
            this.eventSource = new EventSource(`/notify/sse/${this.user_id}`);
            this.eventSource.onmessage = (event) => {
                this.append(JSON.parse(event.data).message);
            }
        } else {
            this.websocket = new WebSocket(`/notify/ws/${this.user_id}`);
...
function sendMessage(event, user_id) {
    ...
    fetch(`/notify/admin/send_notification/${user_id}`, { 
        method: 'POST',
        headers: {
